...
- The ledger follows the project-application card design: a compact name/status toolbar with the primary create action, a responsive three-column card wall, and an inline create card. Each card shows icon, name, status, template, description, creation time, edit/status actions, and the primary open action. Application cards reuse `jetlinks-web-core/src/components/CardBox/CardSummary.vue`; unsupported gateway/camera metrics and the template filter are not rendered.
