...
- Video backend facts: the bind drawer queries gateways through `POST /media/device/_query`, then queries channels only after a gateway click through `POST /media/device/{gatewayId}/channel/_query`. `MediaChannelController` correlates channel permission to `DeviceAssetType.device` via `deviceId`. No separate media-channel asset type is present, so video binding uses the existing `/assets/bind/device` and `/assets/unbind/device` target `business_application` path against each selected channel's related `deviceId`.

Implementation steps:

1. Add typed API wrappers for project member candidates, role CRUD with `applicationId`, paged device details, media gateways and per-gateway channels, and reusable asset binding/unbinding where needed.
...
- Video: opening the camera drawer queries gateways through `/media/device/_query` only. No channel query is issued until the user clicks a gateway; the candidate table then pages through `/media/device/{gatewayId}/channel/_query`. Bound-camera lookup continues to filter `/media/channel/_query/no-paging` by `dim-assets` on `deviceId`. Because channels correlate to `DeviceAssetType.device`, confirming camera selection converts cached channel rows to their `deviceId` values and reuses the same single-object `/assets/bind/device` contract; unbinding continues through `/assets/unbind/device`.
