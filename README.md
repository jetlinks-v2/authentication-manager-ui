...
- The application-role menu-scope filter passes `node --test views/application-center/Template/Save/menu-config.shared.test.mjs`, covering the temporary acceptance of matching non-`app` owners, exact template/runtime `id` intersection, rejection of same-code/different-id menus, and current-project asset-scope projection. `git diff --check` and the full `pnpm build` pass; the build transforms 22557 modules with only existing asset/chunk/CSS warnings. `RoleManagement.vue` remains at 289 lines, and authenticated role-permission interaction remains pending.
