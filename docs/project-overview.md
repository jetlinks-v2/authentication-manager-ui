...
实施入口：共用 `shared/ResourceItem.vue` / 资源列表展示负责数量、用途、在线状态、零数据和未知值；快捷入口增加任务说明；应用恢复业务描述、状态和创建日期；可视化区分作品和素材；规则卡解释触发—判断—执行；运维保留告警处理并明确未处理口径；公告显示摘要；用量按 `/tenant/me/limits` 的资源与指标嵌套契约展示主要资源。图表、趋势、运行成功率没有真实接口依据时不添加。
...
验证计划：定向 SFC/样式解析、语义数据转换测试、两种部署默认布局无重叠、SaaS/私有化用量卡片显隐、长名称/零值/未知值/高用量及窄卡片浏览器验收。概览存储键升级为 `project-overview-v14`，避免旧缓存继续恢复错误布局。当前可控浏览器访问 9200 会跳转登录；真实会话接口联调待恢复，使用隔离组件数据验收时明确区分于真实项目数据。按用户性能约束不执行全量构建。
