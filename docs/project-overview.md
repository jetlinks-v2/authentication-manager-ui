...
运维与监控保留分组标题、在线状态和告警入口，移除资源健康状态与监控告警标题下的两行额外说明。
...
实施入口：共用 `shared/ResourceItem.vue` / 资源列表展示负责数量、用途、在线状态、零数据和未知值；快捷入口增加任务说明；应用恢复业务描述、状态和创建日期并将卡片描述固定为单行；可视化区分作品和素材；规则卡保留资源项信息，移除下方冗余流程说明；运维保留告警处理并明确未处理口径；公告显示摘要；用量按 `/tenant/me/limits` 的资源与指标嵌套契约展示主要资源。图表、趋势、运行成功率没有真实接口依据时不添加。

布局仍为独立仪表盘卡片：SaaS 首行为 `快捷操作 | 资源使用情况（右上）`，私有化首行仅展示占满 12 栏的快捷操作；后续为 `设备接入 | 可视化 | 运维`、`AI | 规则 | 应用`、`采集连接（私有化） | 公告`。`/tenant/me/limits` 用量卡片及其组件目录入口只在 SaaS 项目下注册，私有化环境不展示也不发起该请求。每个内容项采用名称/数量、说明、状态或下一步三层，统一组件内滚动和容器宽度响应，修正应用与运维初始坐标重叠。详情沿用现有菜单或告警浮层，不新增查询页或编辑层。

验证计划：定向 SFC/样式解析、语义数据转换测试、两种部署默认布局无重叠、SaaS/私有化用量卡片显隐、长名称/零值/未知值/高用量及窄卡片浏览器验收。概览使用固定存储键 `project-overview`，历史版本键由 DashBoardCanvas 迁移。当前可控浏览器访问 9200 会跳转登录；真实会话接口联调待恢复，使用隔离组件数据验收时明确区分于真实项目数据。按用户性能约束不执行全量构建。
